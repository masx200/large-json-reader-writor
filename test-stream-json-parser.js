...
export default class StreamJSONParserTester {
      ...
      if (result.success) {
        ...
        if (result.results?.basic) {
          ...
          if (basic.topLevelKeys && basic.topLevelKeys.length > 0) {
            console.log(`   - 前10个键: ${basic.topLevelKeys.slice(0, 10).join(', ')}`);
          }
          ...
          if (basic.structure && Object.keys(basic.structure).length > 0) {
            console.log('\n🏗️ 结构样本 (前5个):');
            Object.entries(basic.structure).slice(0, 5).forEach(([key, info]) => {
        ...
        if (result.results?.array) {
          ...
          if (array.stats?.elementTypes) {
            console.log('   - 元素类型分布:');
            Object.entries(array.stats.elementTypes).forEach(([type, count]) => {
              console.log(`     * ${type}: ${count}`);
            });
          }
        }
        ...
        if (result.results?.deepPaths) {
          ...
          if (deepPaths.deepPaths && deepPaths.deepPaths.length > 0) {
            ...
            deepPaths.deepPaths
              ...
              .slice(0, 8)
              .forEach((p, index) => {
                console.log(`     ${index + 1}. ${p.path} (深度: ${p.depth}, 类型: ${p.type})`);
  ...
  async testAllFiles() {
    ...
    results.forEach(({ file, result }) => {
      ...
      console.log(`${status} ${file} | ${method} | 基础:${basicSuccess} | 数组:${arraySuccess} | 深度:${deepSuccess} | ${keyCount} keys`);
    });
