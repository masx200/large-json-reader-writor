...
class StreamJSONAnalyzer {
  ...
  generateReport(filePath) {
    ...
    if (this.stats.maxDepth > 5) {
      report +=
        `- **Complex Nested Structure:** File has deep nesting (max depth: ${this.stats.maxDepth})\n`;
    } else {
      report +=
        `- **Simple Structure:** File has shallow nesting (max depth: ${this.stats.maxDepth})\n`;
    }

    if (this.stats.arrays > this.stats.objects) {
      report +=
        `- **Array-Oriented:** More arrays than objects, likely list-type data\n`;
    } else {
      report +=
    ...
    if (this.stats.strings > this.stats.numbers) {
      report +=
        `- **Text-Intensive:** More strings than numbers, contains lots of text content\n`;
    } else {
      report +=
    ...
    if (density > 10) {
      report += `- **High-Density Data:** ~${
        density.toFixed(1)
      } elements per KB\n`;
    } else {
      report += `- **Low-Density Data:** ~${
        density.toFixed(1)
      } elements per KB\n`;
    }
    ...
    for (const filePath of filePaths) {
      try {
        if (fs.existsSync(filePath)) {
          ...
          const outputFileName = `stream-analysis-${
...
if (import.meta.main) {
  async function main() {
    ...
    console.log(
      "🎯 Stream JSON Analyzer - Using stream-json and stream-chain for streaming analysis\n",
    );
    ...
    try {
      ...
      console.log(
        `❌ Failed: ${results.filter((r) => !r.success).length} files`,
      );
...
export default StreamJSONAnalyzer;
